...
from textwrap import dedent  # 导入dedent函数，作用是从字符串的行首删除空白，帮助我们使用"""字符串来编写房间描述；
                             # 如果没有这个函数，使用"""字符串会失败，因为它们在屏幕上缩进的程度与python代码相同


class Scene(object):  # 创建基类，具有所有场景都具有的公共功能
...
class Death(Scene):  # 先创建一个Death类，展示最简单的场景  从Scene中继承
...
class CentralCorridor(Scene):   # 创建中央走廊，游戏开始！游戏场景在Map之前，以便于随时引用它们

    def enter(self):
        print(dedent("""
            The Gothons of Planet Percal #25 have invaded your ship and destroyed your entire crew.
            You are the last surviving member and your last mission is to get the neutron destruct bomb from the Weapons
        ...
        if action == "shoot!":
            ...
            print(dedent("""
                ...
                Lbhe zbgure vf fb sng, jura fur fvgf nebhaq gur ubhfr, fur fvgf nvhaq gur ubhfr.
                The Gothon stops, tries not to laugh, then busts out laughing and can't move.
                While he's laughing you run up and shoot him square in the head putting him down, then jump through 
...
class LaserWeaponArmory(Scene):
    ...
    def enter(self):
        print(dedent("""
            ...
            If you get the code wrong 10 times then the lock closes forever and you can't get bomb.
...
class TheBridge(Scene):
    ...
    def enter(self):
        print(dedent("""
            You burst onto the Bridge with the netron destruct bomb under your arm and surprise 5 Gothons who are trying
            to take control of the ship.
            Each of them has an even uglier clown costume than the lsat.
            They haven't pulled their weapons out yet, as they see the active bomb under your arm and don't want to set 
            it off.
            """))
        ...
        if action == "throw the bomb":
            ...
            print(dedent("""
                ...
                blaster at it.
                You then jump back through the door, punch the close button and blast the lock so the Gothons can't get
                out.
                Now that the bomb is placed you run to the escape pod to get off this in can.
                """))
...
class EscapePod(Scene):
    ...
    def enter(self):
        print(dedent("""
            You rush through the ship desperately trying to make it the escape pod before the whole ship explodes.
            It seems like hardly any Gothons are on the ship, so your run is clear of interference.
            You ger to the chamber with the escape pods, and now need to pick one to take.
...
class Map(object):   # 创建Map类，把每个场景存储在一个字典里，用Map.scene引用字典。字典必须引用场景，所以场景必须先存在。
    ...
    def next_scene(self, scene_name):  # 定义next_scene函数，不过在之前的Engine中已经调用了
        val = Map.scenes.get(scene_name)
        return val

    def opening_scene(self):     # 定义opening_scene函数
